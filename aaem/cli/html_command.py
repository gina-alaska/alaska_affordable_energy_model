...
html_command.py
    ...
    A command for the cli to generate summaries (use word summaries for action)
"""
import pycommand
...
class HtmlCommand(pycommand.CommandBase):
    ...
    run command class
    """
    usagestr = ('usage: summaries <options> path_to_model <tag> ')
    optionList = (
            ('alt_out', ('a', '<path>', "alternate output location")),
            ('force',('f', False, "force overwrite of existing directories")),
           )
    description =('generate html summaries'
                    'options: \n'
                   "  " + str([o[0] + ': ' + o[1][2] + '. Use: --' +\
