...
class SolarPower (AnnualSavings):
    """Solar power component of the Alaska Affordable Energy Model: This module
    estimates the potential reduction in diesel fuel use from installation or
    improvement of photovoltaic (PV) solar generation systems. Proposed solar
    generation is based on an estimated capacity per community. Costs associated
    with the installation of panels and necessary power-house improvements
    are also estimated.
    ...
    def run (self, scalers = {'capital costs':1.0}):
        ...
        if self.average_load < self.comp_specs['average load limit'] or \
           not self.proposed_load > 0:
            ...
            "model did not meet minimum generation requirements")
            self.was_run = False
    ...
    def calc_proposed_generation (self):
        ...
        ----------
        Proposed Load: float
             Proposed solar power load (kW)
        generation_proposed : float
            Proposed solar power generation (kWh/yr)
        """
    ...
    def calc_maintenance_cost (self):
        ...
        maintenance_cost : float
            maintenance cost as a percentage of the capital costs
        """
    ...
    def calc_capital_costs (self):
        ...
        powerhouse_cost = 0
        if not self.cd['switchgear suitable for renewables'] and \
